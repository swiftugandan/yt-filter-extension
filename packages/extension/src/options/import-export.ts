...
export async function populateExport(): Promise<void> {
  ...
  const exportObj = {
    ...
    classifierBackend: cfg.classifierBackend || "off",
...
export function setupImportExport(): void {
  ...
  $("#applyImportBtn").addEventListener("click", async () => {
    ...
    cfg.classifierBackend =
      (parsed.classifierBackend as typeof cfg.classifierBackend) ??
      cfg.classifierBackend;
    cfg.serverUrl = (parsed.serverUrl as string) ?? cfg.serverUrl;
    cfg.filters = {
      ...cfg.filters,
